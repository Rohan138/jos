...
sys_exofork(void)
{
	...
	struct Env* e = NULL;
	int ret = env_alloc(&e, curenv ? curenv->env_id : 0);
	if (ret == 0){
...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	int ret = envid2env(envid, &e, 1);
	if(ret < 0) return ret;
	ret = user_mem_check(e, (void *) tf, sizeof(struct Trapframe), PTE_U);
	if(ret < 0) return ret;
	tf->tf_cs |= 3;
	tf->tf_eflags |= FL_IF;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if (!(perm & (PTE_P | PTE_U)) || (perm & ~PTE_SYSCALL)) return -E_INVAL;
	struct Env* e = NULL;
	int ret = envid2env(envid, &e, 1);
	if(ret < 0) return ret;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if(srcva < (void*) UTOP){
		...
		if(target->env_ipc_dstva < (void*) UTOP){
			r = page_insert(target->env_pgdir, page, target->env_ipc_dstva, perm);
			if(r < 0) return r;
